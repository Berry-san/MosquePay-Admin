...
export default function DeleteNeedy() {
    ...
    const [modal, setModal] = useState({
      ...
      const handleDelete = () =>{
        try{
            const res = axios.post(WEB_BASE + 'delete_needy_category',
  ...
  return (
    <div>
        ...
        <Modal isOpen={modal.add} toggle={() => setModal({ add: true })} className="modal-dialog-centered " size="lg">
            ...
            <h1 className=' text-center pt-3 pb-3'>Are you sure you want to delete <span className=' uppercase'>{name}</span>?</h1>
