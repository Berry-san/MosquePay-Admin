...
export default function QuestionUpdate() {
    ...
    useEffect (( ) => {
        axios.get(WEB_BASE + `single_question_category/${id}`, {headers: {'x-api-key': '987654'}})
                .then(res => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        try{
            axios.post(WEB_BASE + `update_question_category/${id}`, 
            qs.stringify({category: category, description: description, admin_id: admin_idd}),
  ...
  return (
          ...
          <h3 className=' text-center text-green-800'> Create Question Category</h3>
              ...
              <FormGroup>
                  ...
                  <input type="text" required value={category} onChange={(e) => setCategory(e.target.value)} className=' w-full mb-3 p-3 rounded-md input-round bg-gray-100 '/>
              ...
              <FormGroup>
                  ...
                  <input type="text" required value={description} onChange={(e) => setDescription(e.target.value)} className=' w-full mb-3 p-3 rounded-md input-round bg-gray-100 '/>
               </FormGroup>
              </Col>
